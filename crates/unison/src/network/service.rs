use super::{NetworkError, StreamHandle, SystemStream};
...
pub trait Service: SystemStream {
    ...
    /// メタデータ抽出付き構造化データの受信
    async fn receive_with_metadata(
        &mut self,
    ) -> Result<(serde_json::Value, HashMap<String, String>), NetworkError> {
        ...
        self.send_with_metadata(
            ...
            HashMap::from([
                (
                    ...
                    "service_heartbeat_start".to_string(),
                ),
    ...
    async fn shutdown(&mut self) -> Result<(), NetworkError> {
        ...
        self.send_with_metadata(
            shutdown_data,
            HashMap::from([("message_type".to_string(), "service_shutdown".to_string())]),
        )
        .await?;
    ...
    fn metadata(&self) -> HashMap<String, String> {
        HashMap::from([
            ...
            (
                ...
                self.stats.requests_processed.to_string(),
            ),
        ])
    }
