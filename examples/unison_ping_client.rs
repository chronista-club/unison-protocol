...
async fn run_unison_tests(client: &mut Box<dyn UnisonClient>) -> Result<()> {
    ...
    if let Some(echoed_data) = response.get("echoed_data") {
        ...
        info!("   Original keys: {:?}", complex_data.as_object().unwrap().keys().collect::<Vec<_>>());
        info!("   Echoed keys: {:?}", echoed_data.as_object().unwrap().keys().collect::<Vec<_>>());
    }
    ...
    info!("   • Throughput: {:.1} pings/sec", 20.0 / total_perf_time.as_secs_f64());
    ...
    if let Some(uptime) = final_response.get("uptime_seconds").and_then(|v| v.as_u64()) {
