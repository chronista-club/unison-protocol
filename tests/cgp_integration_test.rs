...
use unison_protocol::context::{
    CgpProtocolContext, TransportLayer, ServiceRegistry, MessageHandler,
    HandlerRegistry, Handler, UnisonContextBuilder,
    QuicTransportAdapter, ServiceRegistryAdapter,
};
use unison_protocol::network::{NetworkError, ProtocolMessage, MessageType};
...
async fn test_handler_registry() {
    ...
    impl Handler for TestHandler {
        fn handle(&self, _payload: Value) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, NetworkError>> + Send + '_>> {
            Box::pin(async move {
                Ok(serde_json::json!({
                    "result": "test_success"
                }))
