...
async fn test_certificate_loading() -> Result<()> {
    ...
    assert!(cert_result.is_ok(), "Auto certificate loading should succeed");
...
async fn test_embedded_certificates_integration() -> Result<()> {
    ...
    match embedded_result {
        Ok((certs, _key)) => {
            ...
            assert!(!certs.is_empty(), "Embedded certificate chain should not be empty");
        ...
        Err(e) => {
            ...
            assert!(auto_result.is_ok(), "Auto certificate loading should work as fallback");
...
async fn test_build_time_certificate_generation() -> Result<()> {
    ...
    if cert_path.exists() && key_path.exists() {
        ...
        use unison_protocol::network::quic::QuicServer;
        let file_result = QuicServer::load_cert_from_files("assets/certs/cert.pem", "assets/certs/private_key.der");
        
        assert!(file_result.is_ok(), "Loading certificates from external files should work");
        let (certs, _key) = file_result?;
        assert!(!certs.is_empty(), "Certificate chain should not be empty");
        
        info!("📜 Successfully loaded {} certificate(s) from files", certs.len());
    } else {
